...
RELEASE = 1
SPEC = rpmbuild/SPECS/puppet-yum.spec
...
docs: puppet-yum-documentation.pdf

puppet-yum-documentation.pdf: DOCUMENTATION.md
	pandoc DOCUMENTATION.md -o 'puppet-yum-documentation.pdf'
...
$(RPM): $(SPEC) $(SOURCE)
	...
	mv rpmbuild/RPMS/noarch/puppet-yum-$(VERSION)-$(RELEASE).*.rpm $(RPM)
...
$(SRPM): $(SPEC) $(SOURCE)
	...
	#mv rpmbuild/SRPMS/puppet-yum-$(VERSION)-$(RELEASE).*.src.rpm $(SRPM)

#
#	spec
#
$(SPEC): rpmbuild/ puppet-yum.spec.in
	@echo Running templater...
	#cat puppet-yum.spec.in > $(SPEC)
	sed -e s/__VERSION__/$(VERSION)/ -e s/__RELEASE__/$(RELEASE)/ < puppet-yum.spec.in > $(SPEC)
...
$(SOURCE): rpmbuild/
	...
	git archive --prefix=puppet-yum-$(VERSION)/ -o $(SOURCE) $(VERSION) 2> /dev/null || (echo 'Warning: $(VERSION) does not exist.' && git archive --prefix=puppet-yum-$(VERSION)/ -o $(SOURCE) HEAD)
